...
const getBooksByCategory = async function (category = 'all') {
    ...
    if (category === 'all') {
        ...
        const promises = categories.map(async cat => {
            try {
                const res = await Promise.race([fetch(`https://www.googleapis.com/books/v1/volumes?q=subject:${cat}&startIndex=${allBooks?.[cat].length}&maxResults=5`), timeoutPromise()])
                const data = await res.json()
                return { cat, data }
            } catch (error) { return { cat, error } }
        })
        ...
        try {
            const response = await Promise.race([fetch(`https://www.googleapis.com/books/v1/volumes?q=subject:${category}&startIndex=${allBooks?.[category].length}&maxResults=20`)
                , timeoutPromise()])
            const { items } = await response.json()
            items.forEach(bookData => allBooks[category].push(createBook(bookData)))
            return true
        } catch { return false }
    }
}
